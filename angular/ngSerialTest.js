...
var convertArrayBufferToString=function(info) {
  return String.fromCharCode.apply(null, new Uint8Array(info.data));
}
...
//NOTE: Currently only logs to console instead of passing to Arduino through serial port
var serialize = function serialize(){
  /*Constructs 3 lists and passes them to the Chrome API Serial writing functions:
      A list of sound module id types
      A list of those sound module's input net ids
      A list of those sound module's output net ids
      The ordering of these lists must be the same such that the same index in every list corresponds to a "row" of data used by the Arduino
  */

  /*
    Construct the 3 lists from a single list of serialized Module Nodes
  */
  var serializedModules = getSerializedModulesNodes();

  //Declare the 3 output lists
  var moduleTypeIds = [];
  var inputNets = [];
  var outputNets = [];

  //Initialize the lists from the modulenodes in serializedModules
  $(serializedModules).each(function(index,module){
    //only taking one net for now
    var modTypeId = module.moduleTypeId;
    moduleTypeIds.push(modTypeId);
    var iNet = module.inputNets[0];
    var oNet = module.outputNets[0];
    if (iNet != null){
      inputNets.push(iNet);
    } else{
      inputNets.push(-1); //denote Input Module's lack of input
    }
    if (oNet != null){
      outputNets.push(oNet);
    } else{
      outputNets.push(-1); //denote Output Module's lack of output
    }
  });

  $textArea.val("module type ids: " + moduleTypeIds + "\ninput nets: " + inputNets + "\noutput nets: "+ outputNets);
}
